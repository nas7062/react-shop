...
export default function Cart({cart,setcart ,checks,setchecks ,buys,setbuys})
{
    ...
    {
        setcart(cart.filter((e)=>e.id !==id)); // cart에서 제거
        setchecks(checks.filter((check) =>check !== id)); // check된 proudcts 
    }
    ...
    const CheckAllHandler = (checkd) =>{ // 전부체크
        if(checkd)
        {
            const checkItem = [];
    ...
    // cart길이와 checks의 길이가 같고 checks의 길이가 0이 아닐때 모두체크 확인.
    const found = checks.map((check) =>{
        return cart.filter((e) => e.id ===check);
    });
